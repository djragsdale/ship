...
const dataTypeIsStruct = (dataType) => dataType.startsWith('struct<');
const getStructNameFromType = (dataType) => dataType.substring(0, dataType.length - 1).replace('struct<', '');
    ...
    for (let i = 0; i < block.length; i += 1) {
      ...
      if (lineIsStatement(line)) {
        switch (line.keyword) {
          ...
          case (keywords.ECHO): {
            // TODO: Throw error if arguments aren't strings
          ...
          case (keywords.LET): {
            if (line.variable.type === 'prop') {
              ...
              const getStructPropFromParent = (propName) => {
                const structName = getStructNameFromType(parent.type);
              ...
              if (dataTypeIsStruct(parent.type) && !getStructPropFromParent(evaledChildName)) {
                errors.reference.structInvalidProp(getStructNameFromType(parent.type), parentName, evaledChildName);
              }

              const evaledChildValue = await evaluateExpression(line.expression);

              if (dataTypeIsStruct(parent.type) && evaledChildValue.type !== getStructPropFromParent(evaledChildName).dataType) {
            ...
            if (mem.type !== evaledVariable.type) {
          ...
          case (keywords.VAR): {
            ...
            if (line.variable.dataType.type === 'struct') {
