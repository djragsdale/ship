...
describe('FUNC line', () => {
  test('should return a statement AST containing the Func name', () => {
    ...
    expect(ast.result.returnType).toBe(typesByTypeKeywords[returnType]);
    expect(ast.result.parameters).toStrictEqual([]);
  });

  test('should allow parameters if used', () => {
    const funcName = 'FuncName8';
    const returnType = types.Boolean;
    const customParamType = 'MyStruct';
    const param0 = '$p0';
    const param1 = '$p1';
    const ast = funcLine.run(`FUNC ${typeKeywords[returnType]}:${funcName} STRING:${param0} ${customParamType}:${param1}`);
    ...
    expect(ast.result.parameters[0].dataType).toBe(types.String);
