...
const evaluate = async (script, {
  ...
  const doBlock = async (block, {
    ...
    const evaluateExpression = async (expr) => {
      ...
      const floatToNumber = f => Number(`${f[0]}.${f[2] || '0'}`);
      ...
      if (expr.type === 'expression') {
        ...
        if (op === operators.POWER) {
          ...
          try {
            ...
            throw new Error(`Syntax Error: Float ${resRight.value} is not a valid exponent.`);
          }

          return {
            type: 'float',
            value: Math.pow(left.value, power),
          };
        }
      }
      ...
      if (expr.type === 'funcCall') {
        ...
        const evaluatedArguments = await Promise.all(expr.arguments.value.map(val => evaluateExpression(val)));
