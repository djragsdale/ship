        ...
        if (op === operators.CONCAT) {
          const safeConcat = (val) => {
            ...
            if (val.type === types.Float) {
              ...
              return `${val.value[0]}${val.value[1]}${val.value[2].join('')}`;
            }

            if (val.type === types.Array) {
              return `[${val.value.map(safeConcat).join(', ')}]`;
            }
          ...
          return {
            type: types.String,
            value: `${safeConcat(left)}${safeConcat(right)}`,
          };
        }
      }
