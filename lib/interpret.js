    ...
    for (let i = 0; i < block.length; i += 1) {
      ...
      if (lineIsStatement(line)) {
        switch (line.keyword) {
          ...
          case (keywords.IF): {
            const evaluatedExpression = (await evaluateExpression(line.condition)).value;
            if (evaluatedExpression) {
              await doBlock(line.block, { memScope, refs });
            } else if (line.elseBlock) {
              await doBlock(line.elseBlock, { memScope, refs });
            }
            break;
          }
          ...
          case (keywords.LET): {
            if (line.variable.type === 'prop') {
              ...
              // Is there a reason this wouldn't work for arrays?
              // if (isArray(parent) || isRawArray(parent)) {
              //   // 1. Verify evaluated child
              // }
          ...
          case (keywords.VAR): {
            ...
            if (line.variable.dataType.type === types.Struct) {
              ...
              memory.malloc(memScope, line.variable.value, line.variable.dataType);
            }
