...
describe('LET line', () => {
  ...
  test('should allow assigning an array of primitives', () => {
    ...
    expect(ast.result.operator.type).toBe('operator');
    expect(ast.result.operator.value).toBe(operators.ASSIGN);
    expect(ast.result.expression.type).toBe(types.Array);
  ...
  test('should allow assigning an array concatenation expression', () => {
    const varName = '$myVariable';
    const newValue1 = 'something';
    const newValue2 = ' else';
    const ast = letLine.run(`LET ${varName} = (["${newValue1}"] & ["${newValue2}"])`);
    ...
    expect(ast.result.operator.value).toBe(operators.ASSIGN);
    expect(ast.result.expression.type).toBe('expression');
    expect(ast.result.expression.left.type).toBe(types.Array);
    expect(ast.result.expression.left.value[0].type).toBe(types.String);
    expect(ast.result.expression.left.value[0].value).toBe(newValue1);
    expect(ast.result.expression.operator.type).toBe('operator');
    expect(ast.result.expression.operator.value).toBe(operators.CONCAT);
    expect(ast.result.expression.right.type).toBe(types.Array);
    expect(ast.result.expression.right.value[0].type).toBe(types.String);
    expect(ast.result.expression.right.value[0].value).toBe(newValue2);
  });
  ...
  test('should allow assigning a string char method', () => {
    ...
    expect(ast.result.expression.arguments.value[0].type).toBe(types.Float);
    expect(ast.result.expression.arguments.value[0].value).toStrictEqual(['3', null, null]);
  ...
  test('should allow assigning a string slice method', () => {
    ...
    expect(ast.result.expression.arguments.value[0].type).toBe(types.Float);
    expect(ast.result.expression.arguments.value[0].value).toStrictEqual(['3', null, null]);
    expect(ast.result.expression.arguments.value[1].type).toBe(types.Float);
    expect(ast.result.expression.arguments.value[1].value).toStrictEqual(['8', null, null]);
    expect(ast.result.expression.parent.type).toBe(types.String);
    expect(ast.result.expression.parent.value).toBe(newValue);
    expect(ast.result.expression.operator.type).toBe('operator');
    expect(ast.result.expression.operator.value).toBe(operators.ACCESS);
  ...
  test('should allow assigning an array slice method', () => {
    ...
    expect(ast.result.expression.method).toBe('slice');
    expect(ast.result.expression.arguments.value[0].type).toBe(types.Float);
    expect(ast.result.expression.arguments.value[0].value).toStrictEqual(['1', null, null]);
    expect(ast.result.expression.parent.type).toBe(types.Array);
  ...
  test('should allow assigning to an array index by primitive float', () => {
    const varName = '$myArr';
    const index = '2';
    const newValue = 'one';
    const ast = letLine.run(`LET ${varName}[${index}] = "${newValue}"`);

    expect(ast.isError).toBe(false);
    expect(ast.result.variable.type).toBe('prop');
    expect(ast.result.variable.parent.type).toBe('variable');
    expect(ast.result.variable.parent.value).toBe(varName);
    expect(ast.result.variable.prop.type).toBe(types.Float);
    expect(ast.result.variable.prop.value).toStrictEqual(['2', null, null]);
    expect(ast.result.operator.type).toBe('operator');
    expect(ast.result.operator.value).toBe(operators.ASSIGN);
    expect(ast.result.expression.type).toBe(types.String);
    expect(ast.result.expression.value).toBe(newValue);
  ...
  test('should allow assigning to an array index by variable', () => {
    const varName = '$myArr';
    const propName = '$i';
    const newValue = 'one';
    const ast = letLine.run(`LET ${varName}[${propName}] = "${newValue}"`);

    expect(ast.isError).toBe(false);
    expect(ast.result.variable.type).toBe('prop');
    expect(ast.result.variable.parent.type).toBe('variable');
    expect(ast.result.variable.parent.value).toBe(varName);
    expect(ast.result.variable.prop.type).toBe('variable');
    expect(ast.result.variable.prop.value).toBe(propName);
    expect(ast.result.operator.type).toBe('operator');
    expect(ast.result.operator.value).toBe(operators.ASSIGN);
    expect(ast.result.expression.type).toBe(types.String);
    expect(ast.result.expression.value).toBe(newValue);
  ...
  test('should allow assigning a struct', () => {
    const varName = '$myVariable';
    const structName = 'MyStruct';
    const ast = letLine.run(`LET ${varName} = ${structName}`);
    ...
    expect(ast.result.operator.type).toBe('operator');
    expect(ast.result.operator.value).toBe(operators.ASSIGN);
    expect(ast.result.expression.type).toBe('struct');
    expect(ast.result.expression.value).toBe(structName);
  });

  test('should allow assigning to a struct prop', () => {
    const varName = '$myVariable';
    ...
    expect(ast.result.variable.parent.type).toBe('variable');
    expect(ast.result.variable.parent.value).toBe(varName);
    expect(ast.result.variable.prop.type).toBe(types.String);
    expect(ast.result.variable.prop.value).toBe(propName);
    expect(ast.result.operator.type).toBe('operator');
    expect(ast.result.operator.value).toBe(operators.ASSIGN);
    expect(ast.result.expression.type).toBe(types.String);
